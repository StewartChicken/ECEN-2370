...
[[maybe_unused]] static LTDC_HandleTypeDef hltdc;
[[maybe_unused]] static RCC_PeriphCLKInitTypeDef  PeriphClkInitStruct;
[[maybe_unused]] static FONT_t *LCD_Currentfonts;
[[maybe_unused]] static uint16_t CurrentTextColor   = 0xFFFF;

[[maybe_unused]] static SPI_HandleTypeDef SpiHandle;
uint32_t SpiTimeout = SPI_TIMEOUT_MAX; /*<! Value of Timeout when SPI communication fails */

//Someone from STM said it was "often accessed" a 1-dim array, and not a 2d array. However you still access it like a 2dim array,  using fb[y*W+x] instead of fb[y][x].
uint16_t frameBuffer[LCD_PIXEL_WIDTH*LCD_PIXEL_WIDTH] = {0};			//16bpp pixel format.

//static void MX_LTDC_Init(void);
//static void MX_SPI5_Init(void);
[[maybe_unused]] static void SPI_MspInit(SPI_HandleTypeDef *hspi);
[[maybe_unused]] static void SPI_Error(void);
  ...
  for(int16_t y=-radius; y<=radius; y++)
    {
        for(int16_t x=-radius; x<=radius; x++)
        {
            ...
            {
              uint16_t xCoor = x + Xpos;
              uint16_t yCoor = y + Ypos;
              
            	LCD_Draw_Pixel(xCoor, yCoor, color);
            }
        }
    }
}
...
void LCD_update_frame(uint16_t xCoor, uint8_t time)
{
    LCD_Clear(0, LCD_COLOR_WHITE);
    LCD_Draw_Circle_Fill(xCoor, 150, 10, LCD_COLOR_BLACK);
    LCD_display_time(time);
}

void LCD_display_time(uint8_t time)
{
    LCD_SetTextColor(LCD_COLOR_BLACK);
	  LCD_SetFont(&Font16x24);
    ...
    if(time <= 9)
    {
        LCD_DisplayChar(195, 290, time + 0x30);
    } 
    else if(time > 9 && time < 100) 
    {
        LCD_DisplayChar(195, 290, (time % 10) + 0x30);
        LCD_DisplayChar(180, 290, (time / 10) + 0x30);
    }
    else if(time > 99) 
    {
        LCD_DisplayChar(195, 290, (time % 100) % 10 + 0x30);
        LCD_DisplayChar(180, 290, ((time / 10) % 10) + 0x30);
        LCD_DisplayChar(165, 290, (time / 100) + 0x30);
    }

    LCD_DisplayChar(210, 290, 's'); // Display unit
