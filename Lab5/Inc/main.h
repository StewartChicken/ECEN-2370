...
// TODO - the last thing I did was create a general structure for the code. I removed the button and led driver code because, for the time being, it
// serves no purpose. The LED code does nothing for this lab but it will need to be re-included later to satisfy the requirements.
// The button driver acts as a reset for the gyro and leds (when the leds are added back). The next thing that needs to be done is the configuration
// of the gyro driver (registers, macros, pins, etc) itself. The gyro can then be initialized and controlled by the application code.
// To start this task, look to the rm0090 documentation to find the register map of the gyro (it will likely be under the chip name itself). It may also
// be under the SPI5 driver (look at this to figure out which registers need to be configured)
